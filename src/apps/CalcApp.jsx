...
function formatDecimalString(val) {
  if (!val) return '';
...
// COMPONENTE DE INPUT INTELIGENTE (Resolve o problema do ponto/vírgula e do cursor pulando)
const SmartInput = ({ value, onChange, onKeyDown, placeholder, inputRef }) => {
  const localRef = useRef(null);
  const refToUse = inputRef || localRef;

  const handleChange = (e) => {
    const input = e.target;
    const originalSelection = input.selectionStart;
    const formatted = formatDecimalString(input.value);
    
    onChange(formatted);

    // Garante que o cursor permaneça no mesmo lugar após a troca de caracteres
    setTimeout(() => {
      if (refToUse.current) {
        refToUse.current.setSelectionRange(originalSelection, originalSelection);
      }
    }, 0);
  };
...
export function CalcApp() {
  ...
  // Atualização de Protocolo Automática ao Alternar Medicamentos
  useEffect(() => {
    setAmt(formatDecimalString(String(drug.defaultAmt)));
    setVol(formatDecimalString(String(drug.defaultVol)));
    setTargetDose(formatDecimalString(String(drug.doseMin)));
    setFlow('');
  }, [drug]);
  ...
  return (
    <>
      <style>{`
        ...
        .simm-wrapper select { width: 100%; background: transparent; border: none; color: var(--text-primary); font-size: 17px; font-weight: 700; outline: none; appearance: none; }
        ...
        <div className="glass-shell">
            ...
            <div className="fade-engine">
              ...
              <div className="grid-2">
                <div className={`input-box ${!drug.isWeightBased ? 'disabled' : ''}`}>
                  ...
                  <div className="input-wrapper">
                    <SmartInput inputRef={weightRef} value={weight} onChange={setWeight} onKeyDown={(e) => handleKeyDown(e, targetDoseRef)} />
              ...
              <div className="grid-2">
                <div className="input-box">
                  ...
                  <div className="input-wrapper">
                    <SmartInput value={amt} onChange={setAmt} />
                ...
                <div className="input-box">
                  ...
                  <div className="input-wrapper">
                    <SmartInput value={vol} onChange={setVol} />
              ...
              <div className="grid-2">
                <div className="input-box">
                  ...
                  <div className="input-wrapper">
                    <SmartInput inputRef={targetDoseRef} value={targetDose} onChange={(val) => { setTargetDose(val); setFlow(''); }} onKeyDown={(e) => handleKeyDown(e, flowRef)} placeholder={formatReportNumber(drug.doseMin)} />
                  </div>
                ...
                <div className="input-box">
                  ...
                  <div className="input-wrapper">
                    <SmartInput inputRef={flowRef} value={flow} onChange={(val) => { setFlow(val); setTargetDose(''); }} placeholder="mL/h" />
          ...
          {activeTab === 'vis' && (
            <div className="fade-engine">
              ...
              <div className="grid-2">
                <div className="input-box"><span className="input-label">Nora (mcg/kg/min)</span><div className="input-wrapper"><SmartInput value={visValues.noradrenalina} onChange={val => setVisValues({ ...visValues, noradrenalina: val })} placeholder="0,0" /></div></div>
                <div className="input-box"><span className="input-label">Adrenalina (mcg/kg/min)</span><div className="input-wrapper"><SmartInput value={visValues.adrenalina} onChange={val => setVisValues({ ...visValues, adrenalina: val })} placeholder="0,0" /></div></div>
              </div>
              
              <div className="grid-2">
                <div className="input-box"><span className="input-label">Vasopressina (UI/min)</span><div className="input-wrapper"><SmartInput value={visValues.vasopressina} onChange={val => setVisValues({ ...visValues, vasopressina: val })} placeholder="0,0" /></div></div>
                <div className="input-box"><span className="input-label">Dobutamina (mcg/kg/min)</span><div className="input-wrapper"><SmartInput value={visValues.dobutamina} onChange={val => setVisValues({ ...visValues, dobutamina: val })} placeholder="0,0" /></div></div>
              </div>

              <div className="grid-2">
                <div className="input-box"><span className="input-label">Milrinona (mcg/kg/min)</span><div className="input-wrapper"><SmartInput value={visValues.milrinona} onChange={val => setVisValues({ ...visValues, milrinona: val })} placeholder="0,0" /></div></div>
                <div className="input-box"><span className="input-label">Dopamina (mcg/kg/min)</span><div className="input-wrapper"><SmartInput value={visValues.dopamina} onChange={val => setVisValues({ ...visValues, dopamina: val })} placeholder="0,0" /></div></div>
              </div>
